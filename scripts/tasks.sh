...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --upsert)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --upsert requires a value" >&2; exit 2; }
      upsert_id="$1"
      shift
      ;;
    --type)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --type requires a value" >&2; exit 2; }
      upsert_type="$1"
      shift
      ;;
    --title)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --title requires a value" >&2; exit 2; }
      upsert_title="$1"
      shift
      ;;
    --source)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --source requires a value" >&2; exit 2; }
      upsert_source="$1"
      shift
      ;;
    --blocked-by)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --blocked-by requires a value" >&2; exit 2; }
      upsert_blocked_by="$1"
      shift
      ;;
    --notes)
      shift
      [[ $# -gt 0 ]] || { echo "tasks: --notes requires a value" >&2; exit 2; }
      upsert_notes="$1"
      shift
      ;;
    -h|--help)
...
if [[ -n "$upsert_id" ]]; then
  ...
  if [[ "$found" -eq 0 ]]; then
    [[ -n "$upsert_type" ]] || { echo "tasks: --type is required for new task '$upsert_id'" >&2; exit 2; }
    [[ -n "$upsert_title" ]] || { echo "tasks: --title is required for new task '$upsert_id'" >&2; exit 2; }
    [[ -n "$upsert_source" ]] || { echo "tasks: --source is required for new task '$upsert_id'" >&2; exit 2; }
    [[ -n "$upsert_status" ]] || { echo "tasks: --status is required for new task '$upsert_id'" >&2; exit 2; }
