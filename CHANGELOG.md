...
- Reduced scheduled `rust-hygiene` workflow runtime by replacing source-built `cargo install` tool bootstrap with pinned prebuilt binary installation for `cargo-udeps`, `cargo-msrv`, and `cargo-semver-checks`.
...
- Fixed CI-only daemon scenario flakiness in `same_process_retry_after_terminal_save_failure_is_not_stuck_in_progress` by making the observed HTTP test servers count and serve requests per HTTP request instead of per accepted socket, which keeps retry assertions stable when the client reuses connections.
...
- Fixed flaky connector-pool cooldown test `scale_up_respects_threshold_and_cooldown` by increasing test cooldown to remove timing-sensitive extra scale-up under CI scheduler variance.
- Fixed flaky daemon scenario test `duplicate_submit_during_inflight_reasoning_does_not_double_execute_provider` by waiting (bounded) for the first observed provider request before asserting duplicate-submit call count.
